...
public final class ItemsContainer<T extends Item> {
			...
			for (int i = 0; i < data.length; i++) {
				if (data[i] != null) {
					if (data[i].getItemId() == item.getItemId()) {
						data[i] = new Item(data[i].getItemId(), data[i].getItemAmount() + item.getItemAmount());
						return true;
					}
				}
			}
			...
			if (data[i] != null) {
				if (data[i].getItemId() == item.getItemId()) {
					...
					if (amt > toRemove) {
						...
						amt -= toRemove;
						toRemove = 0;
						data[i] = new Item(data[i].getItemId(), amt);
						return removed;
					} else {
						removed += amt;
						toRemove -= amt;
	...
	public boolean containsOne(T item) {
		for (Item aData : data) {
			if (aData != null) {
				if (aData.getItemId() == item.getItemId()) {
					return true;
				}
			}
		}
		...
		if (data[preferredSlot] != null) {
			if (data[preferredSlot].getItemId() == item.getItemId()) {
				...
				if (amt > toRemove) {
					...
					set2(preferredSlot, new Item(data[preferredSlot].getItemId(), amt));
					return removed;
				} else {
					removed += amt;
					toRemove -= amt;
			...
			if (data[i] != null) {
				if (data[i].getItemId() == item.getItemId()) {
					...
					if (amt > toRemove) {
						...
						// amt);
						set2(i, new Item(data[i].getItemId(), amt));
						return removed;
					} else {
						removed += amt;
						toRemove -= amt;
	...
	private boolean hasSpaceForItem(T item) {
		if (alwaysStackable || item.getDefinition().isStackable() || item.getDefinition().isNoted()) {
			...
			if (item.getItemAmount() > 1) {
				return freeSlots() >= item.getItemAmount();
			}
		}
		int index = freeSlot();
		return index != -1;
