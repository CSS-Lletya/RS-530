...
public class ClanManager {
				...
				for (Clan c : clans) {
					if (c != null) {
						if (c.getClanOwner().equals(owner)) {
							...
							c.addUser(p);
							updateClan(c);
							p.getActionSender().sendMessage("Now talking in channel : " + Misc.formatPlayerNameForDisplay(c.getClanName() + ":clan:"));
	...
	public void updateClan(Clan c) {
		for (ClanUser cu : c.getUserList()) {
				...
				spb.addLong(Misc.playerNameToLong(c.getClanOwner()));
				spb.addLong(Misc.playerNameToLong(c.getClanName()));
				...
				for (ClanUser list: c.getUserList()) {
					...
					spb.addLong(Misc.playerNameToLong(p.getUsername()));
	 ...
	 public void newClanMessage(Clan c, ChatMessage m) {
		 ...
		 if (!c.getClanOwner().equals(p.getUsername())) {
				if (c.getTalkRights() > -1) {
					if (c.getTalkRights() == 0) {
						if (!c.isFriendOfOwner(p) && !c.userHasRank(Misc.longToPlayerName(p.getPlayerDetails().getUsernameAsLong()))) {
		 ...
		 for (ClanUser cu : c.getUserList()) {
			...
			spb.addLong(Misc.playerNameToLong(Misc.formatPlayerNameForDisplay(m.getPlayer().getUsername())));
			...
			int offset = 1 + Misc.encryptPlayerChat(chatStr, 0, 1, message.length(), message.getBytes());
	...
	public void openClanSetup(Player p) {
		...
		p.getActionSender().modifyText(Misc.formatPlayerNameForDisplay(clan.getClanName()), 590, 22);
