...
public class Bank {
	...
	public void openPinSettings(int i) {
		...
		if (i == 5 || isPinPending()) {
			...
			p.getActionSender().modifyText("in another " + (recoveryDelay - Misc.getDaysFromMillis(lastPinChange)) + " days.", 14, 45);
			p.getActionSender().modifyText("", 14, 46);
	...
	public void verifyPin(boolean verified) {
		int i = recoveryDelay - Misc.getDaysFromMillis(lastPinChange);
		if (!verified) {
			p.getActionSender().showChildInterface(14, 87, true);
			p.getActionSender().showChildInterface(14, 89, true);
			p.getActionSender().showChildInterface(14, 91, true);
	...
	public void openEnterPin() {
		...
		if (lastDeletionRequest != 0) {
			int i = recoveryDelay -  Misc.getDaysFromMillis(lastDeletionRequest);
	...
	public boolean isPinPending() {
		...
		return Misc.getHoursFromMillis(lastPinChange) < hours;
	}
	...
	public void forgotPin() {
		...
		if (lastDeletionRequest == 0) {
			...
			i = recoveryDelay -  Misc.getDaysFromMillis(lastDeletionRequest);
	...
	public void deposit(int invenSlot, int amount) {
		...
		if (inv.getItemInSlot(invenSlot) == item && item != -1) {
			if (isNote) {
				itemToAdd = ItemData.getUnNotedItem(itemToAdd);
