...
public class SpecialAttack {
	...
	public boolean doSpecialAttack(final Actor killer, final Actor target) {
		...
		case 4151: // Whip.
			...
			/*
			 * if (target instanceof Player) { int energy = ((Player) target).getRunEnergy()
			 * / 4; ((Player) killer).setRunEnergy(((Player) killer).getRunEnergy() +
			 * energy); ((Player) target).setRunEnergy(((Player) target).getRunEnergy() -
			 * energy); if (((Player) killer).getRunEnergy() > 100) { ((Player)
			 * killer).setRunEnergy(100); } if (((Player) target).getRunEnergy() < 0) {
		...
		case 5698:
			...
			doubleHit = true;
			/*
			 * if (Misc.random(3) == 0 && CombatFormula.getMeleeHit(p, target) > 0) { if
			 * (damage <= (getMaxHit(20) / 2) && damage2 <= (getMaxHit(20) / 2)) { damage =
			 * (getMaxHit(20) / 2) + Misc.random(getMaxHit(20) / 2); damage2 =
			 * (getMaxHit(20) / 2) + Misc.random(getMaxHit(20) / 2); } } else if
			 * (Misc.random(2) == 0) { damage = 0; damage2 = 0; }
			 */
			...
			/*
			 * if (damage > target.getHp()) { int randomHp = Misc.random(target.getHp());
			 * damage = randomHp; damage2 = target.getHp() - randomHp; } else { int
			 * hpRemaining = target.getHp() - damage; if (damage2 > hpRemaining) { damage2 =
			 * hpRemaining; } }
			 */
			break;
		...
		case 4587: // Dragon scimitar
			...
			killer.animate(451);
			if (target instanceof Player) {
				if (((Player) target).getPrayers().getOverheadPrayer() >= 1
						&& ((Player) target).getPrayers().getOverheadPrayer() <= 3) {
					((Player) target).getPrayers().setOverheadPrayer(0);
					((Player) target).getPrayers().setHeadIcon(-1);
					((Player) target).getActionSender()
							.sendMessage("The Dragon scimitar slashes through your prayer protection!");
		...
		case 11235: // Dark bow
			...
			World.getInstance().submit(new Task(1, true) {
				...
				protected void execute() {
					...
					if (i == 1) {
						this.setDelay(1);
						World.getInstance().submit(new Task(delayHit + 1) {
		...
		World.getInstance().submit(new Task(hhitDelay){
			@Override
			protected void execute() {
				this.stop();
				if (!target.isDead()) {
	...
	private int getRequiredAmount(int weapon) {
		int[] weapons = { 4151, 1215, 1231, 5680, 5698, 1305, 11694, 11696, 11698, 11700, 11730, 1434, 1377, 3204, 4587,
				14484, 1249, 6739, 7158, 8880, 861, 859, 10284, 805, 6724, 11235, 3101, 4153, 10887, 11061, 13902,
				13899, 13905, 13883, 13879 };
		int[] amount = { 50, // Abyssal whip
				25, // Dragon dagger
				25, // Dragon dagger
				25, // Dragon dagger
				25, // Dragon dagger
